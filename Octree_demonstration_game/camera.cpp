...
void Camera::SetParentRotationAngle(glm::vec3 parentAngle)
{

    parent_rotationAngle = parentAngle;
}

void Camera::SetParent_position(glm::vec3 newParentPosition)
{
    parent_position = newParentPosition;
}

void Camera::SetParentRotation(glm::quat newParentRotation)
{
    parentRotation = newParentRotation;
}

void Camera::SetParentVelocity(glm::vec3 newParentVelocity)
{
...
void Camera::SetupViewMatrix(void){
    ...
    */
    ...
    //glm::mat4 trans = glm::translate(glm::mat4(1.0), parent_position) * glm::mat4_cast(parentRotation)  * glm::translate(glm::mat4(1.0), -position_);
    //view_matrix_ *= trans;


    //glm::mat4 trans =  glm::translate(glm::mat4(1.0), -position_);
    // Combine translation and view matrix in proper order
    //view_matrix_ = glm::lookAt(position_ + parent_location, parent_location, current_up);

    
    //std::cout << parent_rotationAngle.y << std::endl;
    //view_matrix_ = glm::lookAt(parent_position + glm::vec3(sin(glm::degrees(parent_rotationAngle.y))*10.0f, 0, cos(glm::degrees(parent_rotationAngle.y)) * 10.0f), parent_position, current_up);
    view_matrix_ = glm::lookAt(parent_position - parentRotation * position_ * glm::inverse(parentRotation), parent_position, current_up);
    // view_matrix_ = glm::lookAt(parent_position - position_, parent_position, current_up);
